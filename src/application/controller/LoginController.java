...
public abstract class LoginController implements EventHandler<ActionEvent>,Initializable {
	...
	public void newAccount() {
		for(File file : (new File(getAccountStoragePath()).listFiles())){
		if(file.getName().equals(usernameField.getText())) {
			showWarning("That account name is already being used!");
			return;
		}}
		File file = new File(getAccountStoragePath()+"/"+usernameField.getText());
		try {
			FileWriter writer = new FileWriter(file);
			writer.write(passwordField.getText());
			writer.close();
			showWarning("successfully created account " +usernameField.getText());
		} catch(IOException e) {
			e.printStackTrace();
		}
	}
